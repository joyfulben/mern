...
import Wishlist from './Wishlist.js'
  export default class AmiiboHomepage extends Component {
    constructor(props){
      ...
      this.deletedAmiibo = this.deletedAmiibo.bind(this)
    }

    async deletedAmiibo(id){
      try {
        let response = await fetch(this.props.baseURL + '/amiibos/' + id, {
          method: 'DELETE'
        })
        let data = response.json()
        const foundAmiibo = this.state.wishlist.findIndex(amiibo => amiibo._id === id)
        const copyWishlist = [...this.state.wishlist]
        copyWishlist.splice(foundAmiibo, 1)
    ...
    async addToList (i) {
      ...
      const likedCharacter = [this.props.mainList[i], ...this.props.wishList]
    ...
    render(){
      ...
      return(
        ...
        { this.state.wishlist.map((amiibo, i) => {
          return (
            <li className={`wish${i} list-group-item`} key={i + ''} onClick={() => this.deletedAmiibo(amiibo._id)}>
            <img  src={amiibo.image} alt='' />
            <p>{amiibo.name}</p>
            </li>
          )
